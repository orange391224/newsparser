    ...
    def get_publishing_date(self, url, doc):
        ...
        PUBLISH_DATE_TAGS = [
            ...
            {'attribute': 'name', 'value': 'published_time_telegram',
             'content': 'content'},
            {'attribute': 'name', 'value': 'news-header__date-date',
             'content': 'content'}
        ]
        for known_meta_tag in PUBLISH_DATE_TAGS:
            ...
            ATTRS = ['class', 'id']
            VALS = ['date', 'timestamp', 'time', '_3OViwiRtR_PFko9i8o9Mov', 'ra-date-published','ng-scope','news-header__date-date','ng-binding ng-scope']
            matches = []
            for attr in ATTRS:
                for val in VALS:
                    found = self.parser.getElementsByTag(doc, attr=attr, value=val)
