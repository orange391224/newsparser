...
def _extractFromHTMLTag(parsedHTML):
    ...
    #class=
    for tag in parsedHTML.find_all(['span', 'p','div'], class_=re.compile("pubdate|timestamp|article_date|articledate|date|news-header__date-date",re.IGNORECASE)):
        dateText = tag.string
        if dateText is None:
            dateText = tag.text
    ...
    tag = parsedHTML.find("dateformat")
    dateText = tag['time']
    if dateText is None:
        dateText = tag.text
